...
void myDisplay()
{
    ...
    /// Rotate the shape from (cx,cy) center
    double cx=0,cy=0;
    ...
    rectangle(1,4,spin1, tx+1, ty,0,scaleFactor);
    glPopMatrix();
...
//    glScaled(scaleFactor, scaleFactor, 1.0); /// Always change scale at the end of point rendering
    rectangle(1,4,0, tx-1, ty,0,scaleFactor);
    glPopMatrix();
...
//    glScaled(scaleFactor, scaleFactor, 1.0); /// Always change scale at the end of point rendering
    rectangle(2,1,0, tx, ty+2,0,scaleFactor);
    glPopMatrix();
...
//
//    right_triangle(8,16,spin2, tx, ty, 0,  1);
//
//    glPopMatrix();
...
////    glRotatef(spin2, 0.0, 0.0, 1.0);
//
//    right_triangle(8,16,spin2, 6, 8, 0, -1);
//
//    glPopMatrix();
    ...
    /// Circle
    glPushMatrix();
    glColor3f(GREEN.r, 1, GREEN.b);
    double c1x=-20,c1y=-20;
    glTranslatef(c1x, c1y, 0);
    glRotatef(spin1, 0.0, 0.0, 1.0);
    glTranslatef(-c1x, -c1y, 0);
    circle(10,10,3,1,true);
    ...
    renderText("Hello Rafsan, 1234567890", -10, 10);
    glPopMatrix();
